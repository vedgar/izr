...
\end{propozicija}
\begin{proof}
	Dodat ćemo još jedan argument funkciji $\f F$, koji neće raditi ništa osim što će povećavati sve mjesnosti za $1$. Precizno, definiramo funkciju $\f F^2$ (različitu od $\f F^1$\!, jer je mjesnost dio identiteta funkcije) s $\f F(x,y):=F(y)$ (za sve $x$ i $y$). Tada jednakosti
\begin{align}
%\SwapAboveDisplaySkip
\label{eq:F2b}
	\f F(x,0)=F(0)&=a=\f C_a^1(x)\text,\\
\label{eq:F2k}
	\f F(x,y+1)=F(y+1)&=\f H\bigl(y,\f F(y)\bigr)=\f H\bigl(y,\f F(x,y)\bigr)\text,
\end{align}
