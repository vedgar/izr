...
\begin{proof}
Po pretpostavci, skup $\{\vec x\in\N^k\mid\f F(\vec x)\not=\f G(\vec x)\}$ je konačan: označimo mu sve elemente (recimo poredane leksikografski) s $\vec c_1,\vec c_2,\dotsc,\vec c_l$. Ako je taj skup prazan, tada je $\f F=\f G$ pa nemamo što dokazivati. Inače je
\begin{equation}
