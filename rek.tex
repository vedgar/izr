...
Treća stavka zapravo govori o tome da su sve identitete $id_{\N^k}$ izračunljive, samo u skladu s točkom~\ref{sec:briup} svaku $id_{\N^k}$ prikazujemo kao $k$ koordinatnih funkcija $\f I_n^k,n\in[1..k]$ s istim ulaznim podacima.
...
Konkretno, htjeli bismo reći: "kompozicija $\f H\circ\f G$ dvije izračunljive funkcije, $\f G^k$ i $\f H^l$, je izračunljiva funkcija". Ali da bi ta kompozicija imala smisla, kodomena od $\f G^k$ bi morala biti $\N^l$. U skladu s rečenim na početku točke~\ref{sec:briup}, to zapravo znači da imamo $l$ izračunljivih koordinatnih funkcija, $\f G_1^k,\dotsc,\f G_l^k$, i svaka od njih daje po jedan ulazni podatak za funkciju $\f H^l$. Drugim riječima, komponiranje više nije nužno binarna operacija: zato kompoziciju označavamo $\f H\circ(\f G_1,\dotsc,\f G_l)$.
...
Za takve funkcije ubuduće ćemo pisati "simboličku definiciju" $\f F^1:=a\pr\f H^2$, no treba imati na umu da je to samo pokrata za $\f F^1:=\bigl(\f C_a^1\pr\f H^2\circ(\f I_2^3,\f I_3^3)\bigr)\circ(\f Z,\f I_1^1)$. Taj način zadavanja funkcije zvat ćemo \emph{degeneriranom primitivnom rekurzijom}.
\end{definicija}
...
Naravno, općenite neograničene petlje su raznolikije i između provjeravanja uvjeta mogu na razne načine mijenjati stanje, ali kao i za primitivnu rekurziju, aksiomatski ćemo pretpostaviti samo izračunljivost vrlo jednostavnih petlji: onih čije tijelo je oblika \t{++i}, inicijalizacija \t{i=0}, a uvjet je neka izračunljiva relacija. Poslije ćemo vidjeti da u tom modelu možemo pisati i sasvim općenite beskonačne petlje.
...
Za obrat teorema~\ref{tm:pir} morat ćemo se više namučiti.
...
Kao što smo već napomenuli, još ne znamo dokazati da je to izračunljivo ako izračunljive funkcije ("grane") $G_i$ nisu totalne. To ćemo kasnije --- zasad dokažimo da je funkcija dobivena grananjem iz \emph{totalnih} izračunljivih funkcija i uvjeta, izračunljiva u funkcijskom modelu.
...
Primijetimo samo da u teoremu~\ref{tm:grek} ne smijemo ispustiti $\f G_0$, jer $\varnothing^k$ nije totalna!
...
Na taj način, $y$ je kao "dinamički $l$": izgubili smo mogućnost rada s parcijalnim funkcijama, ali dobili smo mogućnost da broj koraka izračunavanja prenesemo kao ulazni podatak. Slikovito, u jednom slučaju računamo $G_i(\vec x)$, a u drugom $G(\vec x,i)$, pa možemo reći da smo "dignuli subskript" (ili "spustili superskript", ako se radi o mjesnosti) na razinu ulaznog podatka. U ovoj točki napravit ćemo nekoliko takvih "dinamizacija", za funkcije odnosno familije funkcija koje smo već upoznali, kao i za neke koje još nismo ali se svejedno prirodno i često pojavljuju.
...
\begin{align}
    \f I_{k+1}^{k+1}(\vec x,0)&=0\text,&
    \f I_{k+1}^{k+1}(\vec x,y+1)&=\f{Sc}\bigl(\f I_{k+1}^{k+1}(\vec x,y)\bigr)\text,
\end{align}
odnosno $\f I_{k+1}^{k+1}=\f C_0^k\pr\f{Sc}\circ\f I_{k+2}^{k+2}$, ali ne čini se kao neki dobitak.
\end{primjer}
...
Primijetimo da smo u definiciji mogli napisati znak $:=$ umjesto $:\simeq$, jer je desna strana doista uvijek definirana: relacija $y<H(\vec x)$ je sigurno lažna za neki $y$ (prvi takav je upravo $y:=H(\vec x)$, koji postoji jer je $H$ totalna), pa je kondicional tada istinit. Drugim riječima, $F(\vec x)$ će biti najmanji broj $y$ ispod $H(\vec x)$ koji zadovoljava $R(\vec x,y)$ ako takav postoji, a inače će biti $F(\vec x)=H(\vec x)$.
