...
\end{korolar}
\begin{proof}
Točkovna definicija~\eqref{eq:Russelldef} u simboličkom obliku glasi $\f{Russell}=\f{Sc}\circ\f{comp}_1\circ(\f I_1^1,\f I_1^1)$, pa tvrdnja slijedi po propoziciji~\ref{prop:compspec}.
\end{proof}
...
\end{equation}
dakle $\rus$ je totalna funkcija. To, zajedno s parcijalnom rekurzivnošću, značilo bi da je $\rus$ rekurzivna funkcija, što je u kontradikciji s lemom~\ref{lm:Russellnrek} (jer je $\rus|_{K}=\f{Russell}$).
\end{proof}
...
$\{\t\_,\t a,\t b,\t c,\dotsc,\t z,\t A,\t B,\t C,\dotsc,\t Z,\t 0,\t 1,\t 2,\dotsc,\t 9\}$ koja se sastoji od $63$ znaka, i taj je jezik zadan pravilom da prvi znak ne smije biti dekadska znamenka, a čitava varijabla ne smije biti nijedna od konačno mnogo ključnih riječi.
...
Koristili smo propoziciju~\ref{pp:F1odl} i teorem~\ref{tm:oikr} te činjenicu da podriječ ima manji kod nego riječ u kojoj se nalazi (dokažite to!). Brojni drugi trikovi takvog tipa, uključujući i sasvim netrivijalne stvari poput supstitucije terma za varijablu u formuli, mogu se vidjeti u~\cite{smullyan}.
...
\begin{equation}
    ...
    \end{prog}\text,
\end{equation}
a $\vec u=(1,2)$. Vidimo da $P$-izračunavanje s $\vec u$,
\begin{equation}
...
U prethodnoj točki uveli smo, za svaki prirodni broj $u$, formulu (štoviše, rečenicu) $\varphi_u$ koja je valjana ako i samo ako je $u$ element Russellovog skupa. Također smo uveli strukturu $\mathfrak N=(\N,0,\f{Sc},\dotsc)$, čiji konstantsko-funkcijski fragment se podudara sa standardnim modelom od PA (prirodni brojevi, nula i sljedbenik).
...
\end{multline}
Sheme (A1), (A2) i (A3) možemo opisati konkatenacijom. Napisat ćemo je za (A1) --- ostale su samo dulje ali ne bitno kompliciranije.
\begin{multline}
