...
Srećom, naša relacija je \emph{padajuća} po $t$: jednom kad $t\cdot y$ postane veće od $x$, ostat će veće od $x$ za sve veće $t$. Kod takvih relacija, "prva laž" i "zadnja istina" su susjedne, te jednostavno možemo naći najmanji $t$ za koji je $t\cdot y>x$, i uzeti njegov prethodnik.
...
\end{align}
za sve $i\in[1..l]$, također (primitivno) rekurzivne.
\end{propozicija}
\begin{proof}
...
\begin{definicija}[{name=[kodiranje skupa $\mathcal Ins$]}]\label{def:kodIns}
...
\begin{definicija}[{name=[kodiranje skupa $\mathcal Prog$]}]
...
\begin{propozicija}[{name=[parcijalna rekurzivnost univerzalnih funkcija fiksne mjesnosti]}]\label{prop:compspec}
