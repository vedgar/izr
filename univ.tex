...
Za $(n,i)\in\N^2$, označimo s $\f{plog}(n,i)$ eksponent prim-broja $p_i$ u rastavu broja $n\bump$ na prim-faktore. Funkcija $\f{plog}^2$ je primitivno rekurzivna.
\end{lema}
...
Pomoću funkcije $\f{plog}$ napokon možemo dekodirati proizvoljni kod konačnog niza.
...
ne ovisi o konkretnom RAM-stroju odnosno programu. Mogli bismo npr.\ proći kroz program i naći mu širinu $m_P$, pa kodirati stanje kao konačan niz registara do $\reg{m_P}$, ali to bi ovisilo o programu, i dodavanje potpuno irelevantnih instrukcija koje se možda uopće ne izvrše (\emph{unreachable code}) bi promijenilo kod stanja registara za isto izračunavanje (isti niz konfiguracija), te kodiranje ne bi bilo funkcija.
...
\end{align}
(jasno, \eqref{eq:knkkpj} vrijedi samo ako je $r_i>0$).
...
\section{Indeksi izračunljivih funkcija}
