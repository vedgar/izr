...
Neka je $P^1$ RAM-algoritam širine $m$, te $c=(r_0,r_1,\dots,r_{m-1},0,0,\dots,pc)$ konfiguracija RAM-stroja s programom $P$. \emph{Turing-reprezentacija} od $c$ je konfiguracija sa stanjem $p_{pc}$, trakom oblika $\t\$\bl^k\t\#v\bl\dots$ i pozicijom većom od $k$ --- pri čemu je $v$ riječ nad $B^m$ u čijem $i$-tom tragu piše $\t\textbullet^{r_i}\t\textopenbullet^{\dulj v-r_i}$.
...
Recimo,~\eqref{eq:bitmap} je primjer trake reprezentacije konfiguracije $(1,4,0,5,0,0,\dots)$. Također, zadnja konfiguracija u~\eqref{eq:+afaze} je reprezentacija početne konfiguracije RAM-stroja s programom $P_0$ iz~\eqref{eq:+aprog}, s ulazom \t{ab}.
...
%TODO: spojiti ovu lemu s kasnijom propozicijom
...
Da bismo implementirali prijelaze iz stanja $s_i$, moramo naučiti "adresirati bitove" na traci Turing-reprezentacije. Svaki element od $B^m$ vidimo kao $m$-bitnu riječ, za promjenu pojedinog bita koristeći istu tehniku kao "pravi" procesor: čitavu procesorsku riječ zamijenimo drugom, koja se podudara s njom u svim bitovima osim onog koji želimo promijeniti. Primjerice, ako na traci~\eqref{eq:bitmap} želimo "dekrementirati $\reg1$", zamijenit ćemo njen trinaesti znak devetim znakom iz~\eqref{eq:B4}.
...
\end{propozicija}
\begin{proof}
Zbog $c(\textsc{pc})=i<n=n_P$ konfiguracija $c$ sigurno nije završna, pa postoji instrukcija $I_i=I_{c(\textsc{pc})}$. Konstruiramo traženi fragment ovisno o tipu te instrukcije. Taj fragment počinje u stanju $p_i$, a trebao bi završiti u stanju $p_{i'}$ za $i':=d(\textsc{pc})$ --- no zbog leme~\ref{lm:pi>si} dovoljno je stati u $p_{i'}$, na bilo kojoj poziciji $p>k$, s trakom kakva je u $Turing_{kmn}(d)$.
...
%\end{array}
\end{multline}
	s izlaznim podatkom $9=2\cdot\mspace{1mu}4+1$, i odgovara mu (po preslikavanju $Turing_{224}$) šetnja %kroz Turing-konfiguracije %(ne pišemo, svima zajednički, početak \t{\renewcommand{\arraystretch}{0.1}\$\boo\boo\#})
...
\,\text,
\end{multline}
koja se direktno nastavlja na onu započetu u~\eqref{eq:+afaze}.
...
\renewcommand{\arraystretch}{0.1}
	\T{\$\bl\bl\#}{\mund{s_4}}{\bio}{\bio\bio\bio\bio\bio\bio\bio\bio}\\[-1mm]
