...
\end{equation}
Za dokaz specifikacije, neka je $w=\alpha_0\alpha_1\dotsm\alpha_{l-1}\in\Sigma^*$ proizvoljna ($l:=\dulj{w}$). Tada, ako označimo $n:=\kr w$, vrijedi $\f{lh}(n,b')=l$ i $d_i:=\f{digit}(n,i,b')=\N\Sigma_0(\alpha_i)=\N\Gamma_0(\alpha_i)$ za sve $i<l$. Iz toga slijedi
\begin{equation}
...
\end{equation}
parcijalno je rekurzivna (naravno, $\dom{\f{stop}}=\{\kr w\mid w\in\dom{\varphi_0}\}$).
\end{lema}
\begin{proof}
Tvrdimo da je
...
\renewcommand{\arraystretch}{0.1}
    B^m:=\prod_{i<m}\{\t\textopenbullet,\t\textbullet\}=\bigl\{\left[\begin{array}{@{}l@{}}\beta_0\\
    \MTFlushSpaceAbove
    \vdotswithin{\beta}\\
...
\T{}{q_0}{a}{b}\leadsto^*
\T{\$a}{q_2}{b}{\#}\leadsto^*
\T{\$\boo\boo\#}{\mund{p_0}}{\boi}{\boi\boi\boi}\,\text,
\end{equation}
...
\end{korolar}
\begin{proof}
Samo treba primijeniti upravo dokazani korolar na RAM-program $P_0$ (dakle $f=\N\varphi_0$), na $x:=\kr w$ i $k:=\dulj{w}$. %Štoviše, jer se $f$ računa na $x$ koji jest kod, dovoljno je zahtijevati $f\approx\N\varphi_0$.
\end{proof}
...
Primijetimo da zbog $\kr v=y=\kr{\varphi_0(w)}$, i propozicije~\ref{pp:bijkr}, zapravo vrijedi $v=\varphi_0(w)$, odnosno (ako uopće dođemo u situaciju da možemo primijeniti lemu~\ref{lm:faza4}) već imamo izlaznu riječ na traci. Samo je još potrebno premjestiti je na početak, i obrisati s trake sve ostalo.
...
Primijetite da smo stigli do završnog stanja $q_z$. Time smo napokon završili konstrukciju Turingovog stroja $\mathcal T_0$, i sve je spremno za dokaz "obrata" teorema~\ref{tm:tikp}.
