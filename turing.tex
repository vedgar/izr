...
Za surjektivnost, neka je $x\in\N$ proizvoljan. Prema lemi~\ref{lm:pomakbaza}, postoje znamenke $\vec z\in[1\dd b]^*$ takve da je $x=(\vec z)_b$. Ako sad za svaku $z_i$ označimo $\alpha_i:=\N\Sigma^{-1}(z_i)\in\Sigma$, riječ sastavljena od tih znakova ima upravo kod $x$.
\end{proof}
...
\begin{propozicija}[{name=[treći fragment transpiliranog stroja]}]\label{pp:faza3}
	...
	Postoji fragment Turingovog stroja koji za sve $x\in\N$, počevši od bilo koje $m$-reprezentacije početne $\mathcal S_0$-konfiguracije s ulazom $x$, dostigne neku $m$-reprezentaciju završne $\mathcal S_0$-konfiguracije ako je $x\in\dom{\N\phi_0}$, a inače nikada ne stigne u stanje $p_n$.
\end{propozicija}
\begin{proof}
	Za svaki $x\in\N$, označimo sa $(c_l)_l$ $P_0$-izračunavanje s $x$ i indukcijom po $l$ dokažimo da dosad sastavljeni fragment Turingovog stroja dostigne $m$-reprezentaciju od $c_l$. Baza je očita, jer je $c_0$ upravo početna konfiguracija. %Baza je primjena lema~\ref{lm:faza1} i~\ref{lm:faza2}, kako je ilustrirano u~\eqref{eq:+afaze}. 
...
\shortintertext{s komponentama}
    Q&:=\bdcup_{i\le6}\{q_i\}\dcup\bdcup_{i\le n}\{p_i,s_i,t_i\}\dcup\bdcup_{\mathclap{\alpha\in\Sigma_0\dcup\{\t\$,\bl\}}}\,\{n_\alpha,l_\alpha\}\dcup\{q_x\}\text,\\
