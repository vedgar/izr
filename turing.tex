...
%Ostatak Turingova stroja koji konstruiramo vezat će se s dosad izgrađenim dijelom isključivo kroz stanje $p_n$, a njegov rad će uvijek stati, pa ćemo na kraju moći zaključiti da čitav Turingov stroj stane ako i samo ako dođe u stanje $p_n$, odnosno ako i samo ako je ulazna riječ u domeni od $\varphi_0$.
%\end{napomena}
...
Postoji fragment Turingova stroja koji prevodi bilo koju $m$-reprezentaciju završne konfiguracije $(p_n,k,\t\$\bl^{\dulj w}\t\#t\bl\ldots)$, gdje je $k>\dulj w$ i $t$ u tragu $0$ ima oblik $\t\textbullet^y\t\textopenbullet^{\dulj t-y}$, u konfiguraciju $(l_\bl,\dulj v,\t\$v\bl\ldots)$, gdje je $\kr v=y$.% te $u\in(B^m)^*$.
\end{lema}
...
%\noindent\begin{gather*}
    %\SwapAboveDisplaySkip
%\label{d:81}\tag{$\delta$5}\delta(q_3,\t\#):=\delta(q_6,\gamma):=(q_6,\bl,1)\text{, za sve }\gamma\in B^m\\
...
%Stigli smo do završnog stanja $l_{\t\$}$. Time smo napokon završili konstrukciju Turingova stroja $\mathcal T_0$ i sve je spremno za dokaz "obrata" teorema~\ref{tm:tikp}.
...
\end{align}
	i funkcijom prijelaza $\delta$ zadanom jednakostima~\eqref{d:10}--\eqref{d:50}, %($\delta\,\cdots$), %\eqref{d:11}--\eqref{d:12}, \eqref{d:21}--\eqref{d:28}, \eqref{d:31}--\eqref{d:61}, \eqref{d:71}--\eqref{d:70} i \eqref{d:81}--\eqref{d:89},
