...
\begin{definicija}\label{def:Tcomputefi}
...
\subsection{Kodiranje znakova}\label{sec:kSigma}
...
Najprirodnije kodiranje ograničenih nizova, toliko prirodno da možda nismo ni svjesni kako ga koristimo (za $b'=10$) svaki put kad čitamo i pišemo višeznamenkaste brojeve, je \textbf{zapis u bazi} $b'$. Ipak, s njime postoje dva problema. Prvi problem su početne nule ($(001121)_3=(1121)_3$), što smo riješili tako što smo počeli brojiti kodove znakova od $1$, ali smo na račun toga dobili uključenu gornju granicu. Kako opravdati znamenku $b'$ u bazi $b'$? Drugi problem su jednočlane abecede: znamo da se obični pozicijski brojevni sustavi definiraju samo za baze od $2$ nadalje. Naravno, postoji i unarni zapis, ali on nije pozicijski --- ili jest?
...
\end{multline}
To po tranzitivnosti zapravo znači da smo zapise poredali po duljini, a zapise iste duljine leksikografski (\emph{shortlex ordering}).
\end{proof}
...
\end{equation}
za svaku riječ $w=\alpha_{n-1}\dotsm\alpha_0\in\Sigma^*$ (uz oznake $n:=\left|w\right|$ i $b':=\card\Sigma$).
\end{definicija}
...
% umjesto specifikacije napraviti definiciju s točkicom (b=0->b=1)
Postoje primitivno rekurzivne funkcije $\f{lh}^2$\!, $\f{digit}^3$\!, $\f{concat}^3$ i $\f{prefix}^3$, takve da za sve $b\ge 1$, za sve $i,n,m\in\N$ vrijedi:
\begin{labeling}{$\f{concat}(m,n,b)$}
    ...
    \item[$\f{concat}(m,n,b)$] je broj čiji zapis u pomaknutoj bazi $b$ se dobije konkatenacijom istih takvih zapisa za $m$ i $n$ redom.
...
\end{labeling}
\end{lema}
%Za fiksni $b$, $\f{concat}(m,n,b)$ još pišemo $m\conc b n$, a $\f{Prefix}(m,n,b)$ pišemo $m\preceq_b n$.
\begin{proof}
Za $\f{lh}$, treba uočiti da zapisa duljine $i$ ima točno $b^i$. Dakle, samo trebamo zbrajati takve brojeve dok ne prijeđemo $n$. Također, očito je $\f{lh}(n,b)\le n$.
\begin{equation}
...
Sada jednostavno možemo dobiti konkatenaciju: $m$ pomaknemo udesno da $n$ taman stane u dobiveni prostor, i samo ga pribrojimo.
\begin{equation}
    \f{concat}(m,n,b):=m\conc bn:=m\cdot b^{\f{lh}(n,b)}+n
\end{equation}

Pomoću konkatenacije možemo dobiti $\f{Prefix}$: prefiks je ono što se može konkatenirati s nečim tako da se dobije početni zapis $n$. Očito su dijelovi manji ili jednaki $n$.
\begin{equation}
    \f{prefix}(n,i,b):=(\mu m\le n)(\exists t\le n)\bigl(n=\f{concat}(m,t,b)\land\,\f{lh}(m,b)=i\bigr)
\end{equation}

Sada nije teško napisati ni $\f{digit}$: to je posljednja znamenka prefiksa čija je duljina za jedan veća od pozicije znamenke.
\begin{equation}
    \f{digit}(n,i,b):=\f{mod}_1\bigl(\f{prefix}(n,i+1,b),b\bigr)
\end{equation}

Sve te funkcije su primitivno rekurzivne, koristeći dosadašnje rezultate. \end{proof}

Koristeći upravo dokazano, možemo napisati definiciju prateće funkcije.
...
\section{Parcijalna rekurzivnost jezičnih funkcija}\label{sec:tikp}
...
Slično možemo kodirati i $\Gamma_0$ --- ali kako već imamo kodiranje skupa $\Sigma_0\subset\Gamma_0$, želimo da znakovi ulazne abecede imaju iste kodove. Dakle, označimo $b:=\card\Gamma_0$, i proširimo bijekciju $\N\Sigma_0$ između $\Sigma_0$ i $[1..b']$ na bijekciju $\N\Gamma_0$ između $\Gamma_0$ i $[0..b\rangle$, tako da $\bl$ preslikamo u $0$, a ostale elemente iz $\Gamma_0\setminus\Sigma_0$ bijektivno u skup $\langle b'..b\rangle$.
...
\end{equation}
Za dokaz specifikacije, neka je $w=\alpha_0\alpha_1\dotsm\alpha_{l-1}\in\Sigma^*$ proizvoljna ($l:=\left|w\right|$). Tada, ako označimo $n:=\kr w$, vrijedi $\f{lh}(n,b')=l$ i $d_i:=\f{digit}(n,i,b')=\N\Sigma_0(\alpha_i)=\N\Gamma_0(\alpha_i)$ za sve $i<l$. Iz toga slijedi
\begin{equation}
...
\end{equation}
pa je $\N\varphi_0$ parcijalno rekurzivna kao kompozicija takvih. Sama parcijalna jednakost~\eqref{eq:kodfiprek} je zapravo~\eqref{eq:kodfidef} iz definicije~\ref{def:kodfi}, samo izrečena u "izračunljivom obliku". Za svaki $x\in\N$, imamo dva podslučaja ovisno o tome je li $w:=(\N\Sigma_0^*)^{-1}(x)$ element od $\dom{\varphi_0}$ ili nije.

Ako nije, tada $\N\varphi_0(x)$ nema smisla, a niti desna strana od~\eqref{eq:kodfiprek} nema smisla --- $\mathcal T_0$ računa $\varphi_0$, pa $w\not\in\dom{\varphi_0}$ znači da $\mathcal T_0$-izračunavanje s $w$ ne stane. Tada $x\not\in\dom{\f{stop}}$ prema lemi~\ref{lm:stopprek}\eqref{it:nstop}, a onda po marljivoj evaluaciji~\eqref{eq:domkomp} također $x$ nije ni u domeni desne strane.
...
S druge strane, radnu abecedu već možemo preciznije specificirati. Prvo, u njoj se svakako nalaze svi znakovi iz $\Sigma_0$, kodirani po $\N\Sigma_0$ u brojeve iz $[1..b']$. Znak ${\N\Sigma_0}^{-1}(t)$ označit ćemo s $\alpha_t$. Oni predstavljaju "znamenke" od $1$ do $b'$ u pomaknutoj bazi $b'$. 
...
Sljedeći korak je konstruirati početnu konfiguraciju od $\mathcal S_0$, desno od separatora \t\# na poziciji $\left|w\right|$. Znamo da to mora biti element od $(B^m)^*$, pa pokušajmo s unarnim kodiranjem: uzmimo jedan konkretan element $r_1\in B^m$, i cilj nam je desno od \t\# napisati $r_1^{\langle w\rangle}$. Što je točno $r_1$ (i čitav $B^m$) reći ćemo u idućoj točki, ali zasad ga samo tretiramo kao zasebni znak (baš kao što smo dosad činili s prazninom, za koju smo također nagovijestili da će biti element od $B^m$).
...
\T{\$aaa}{q_2}{\#}{\t{$r_1$}}\leadsto
\T{\$aa}{q_2}{a}{\#\t{$r_1$}}\leadsto
\T{\$a}{q_2}{a}{d\#\t{$r_1$}}\leadsto
\T{\$}{q_2}{a}{dd\#\t{$r_1$}}\leadsto{}\\
{}\leadsto
...
\renewcommand{\arraystretch}{0.1}
    ...
    \Bigr\}\text.
\end{equation}
Konkretno, ako (za $m=4$) u nekom trenutku imamo konfiguraciju $(1,4,0,5,0,0,\dotsc)$, te ako je ulazna riječ bila duljine $7$, traka će izgledati
...
\begin{equation}\label{eq:+aprog}
    ...
    \end{prog}
\end{equation}
čija je dubina $m=2$. Za ulaznu riječ $w_2:=\t{ab}$ imamo $\kr{w_2}=(12)_2=4$, pa je konfiguracija Turingovog stroja kroz faze (početna, nakon prve faze i nakon druge faze)
\begin{equation}\label{eq:+afaze}
\renewcommand{\arraystretch}{0.1}
\T{}{q_0}{a}{b}\leadsto^*
\T{\$a}{q_2}{b}{\#}\leadsto^*
\T{\$\boo\boo\#}{p_0}{\boi}{\boi\boi\boi}\,\text,
\end{equation}
te smo spremni za početak simulacije. 
\end{primjer}

Rekli smo da ćemo vrijednost programskog brojača držati u stanju stroja, i to je već učinjeno kroz subskript stanja $p_0$: ta nula znači da je \textsc{pc} upravo postao $0$. Općenito, ako je $P_0$ duljine $n:=n_{P_0}$, imat ćemo stanja $p_i,i\in[0..n]$, i ulazak u stanje $p_n$ značit će završetak $P_0$-izračunavanja.
...
\end{lema}
\begin{proof}
Vrlo je jednostavno. Dok god čitamo znak iz $B^m$, pomičemo se lijevo~\eqref{d:31}. Kad pročitamo \t\# (što će se sigurno dogoditi nakon $t-k-1$ koraka), pomaknemo se desno~\eqref{d:32}.
...
\begin{primjer}\label{pr:+a2}
U primjeru~\ref{pr:+a} smo vidjeli RAM-program $P_{\t a}$~\eqref{eq:+aprog} koji računa funkciju $\N\varphi_{\t a}(x):=2x+1$,  prateću funkciju dopisivanja slova \t a na kraj riječi. $P_0$ ima duljinu $n=5$ i dubinu $m=2$.

$P_{\t a}$-izračunavanje s $\kr{\t{ab}}=4$ je prikazano tablicom %($c$ skraćeno zapisujemo kao $c(\reg0)c(\reg1)c(\textsc{pc})$)
\begin{equation*}
...
\renewcommand{\arraystretch}{0.1}
\T{}{s_0}{\boi}{\boi\boi\boi}\leadsto^*
\T{}{s_1}{\boi}{\boi\boi}\leadsto^*
\T{}{s_2}{\bii}{\boi\boi}\leadsto^*
\T{}{s_3}{\bii}{\bii\boi}\leadsto^*
\T{}{s_0}{\bii}{\bii\boi}\leadsto^*
\T{}{s_1}{\bii}{\bii}\leadsto^*
\T{}{s_2}{\bii}{\bii\bio}\leadsto^*
\T{}{s_3}{\bii}{\bii\bio\bio}\leadsto^*
\T{}{s_0}{\bii}{\bii\bio\bio}\leadsto^*
{}\\{}\leadsto^*
\renewcommand{\arraystretch}{0.1}
\T{}{s_1}{\bii}{\bio\bio\bio}\leadsto^*
\T{}{s_2}{\bii}{\bio\bio\bio\bio}\leadsto^*
\T{}{s_3}{\bii}{\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_0}{\bii}{\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_1}{\bio}{\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_2}{\bio}{\bio\bio\bio\bio\bio\bio}\leadsto^*
{}\\{}\leadsto^*
\renewcommand{\arraystretch}{0.1}
\T{}{s_3}{\bio}{\bio\bio\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_0}{\bio}{\bio\bio\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_4}{\bio}{\bio\bio\bio\bio\bio\bio\bio}\leadsto^*
\T{}{s_5}{\bio}{\bio\bio\bio\bio\bio\bio\bio\bio}
\,\text,
\end{multline}
koja se prirodno nastavlja na onu započetu u~\eqref{eq:+afaze} (kroz lemu~\ref{lm:pi>si} za $i=0$).

Fragment Turingovog stroja koji odgovara $P_{\t a}$ (treći fragment) je prikazan dijagramom
\begin{equation}
...
Ovaj će postupak biti sličan onome u točki~\ref{sec:tmfaza2}, samo u obrnutom smjeru. Imamo broj $y$ zapisan unarno (pomaknuta baza $1$) u tragu $0$, i trebamo ga zapisati u pomaknutoj bazi $b'$ --- skidajući po jedan \textbullet\ iz traga $0$ i inkrementirajući riječ iz $\Sigma_0^*$ svaki put. Ipak, nekoliko tehničkih detalja čine ovaj postupak kompliciranijim.
...
Spasit ćemo se tako što ćemo za rekonstrukciju izlazne riječi upotrijebiti dio \emph{desno} od separatora, unutar kojeg smo simulirali $P_0$-izračunavanje s $w$. Taj dio će sigurno biti dovoljno dugačak, jer sadrži bar $y=\kr{\varphi_0(w)}$ znakova \textbullet\ u tragu $0$.
...
Primijetimo da se za $b'=1$ postiže jednakost za sve $u$ (jer je jedina moguća znamenka $1$, pa su svi pribrojnici jednaki $1$).
\end{proof}
...
Postoji fragment Turingovog stroja koji prevodi reprezentaciju završne konfiguracije $(s_n,\dulj w+2,\t\$\bl^{\dulj w}\t\#t\bl\ldots)$, gdje $t$ u tragu $0$ ima oblik $\t\textbullet^y\t\textopenbullet^{z-y}$, u konfiguraciju $(q_5,\dulj w+1,\t\$\bl^{\dulj w}\t\#uv\t\$\bl\ldots)$, gdje je $\kr v=y$, te $u\in(B^m)^{z-\left|v\right|}$.
\end{lema}
\begin{proof}
...
Primijetimo da zbog $\kr v=y=\kr{\varphi_0(w)}$, i propozicije~\ref{pp:bijkr}, zapravo vrijedi $v=\varphi_0(w)$, odnosno (ako uopće dođemo u situaciju da možemo primijeniti lemu~\ref{lm:faza4}) već imamo traženu riječ na traci. Samo je još potrebno premjestiti je na početak trake, i obrisati s nje sve ostalo.
...
\end{lema}
\begin{proof}
Prvi korak se sastoji u brisanju svih ostataka izračunavanja, i separatora, tako da jedino "prežive" izlazna riječ $v$ i dva graničnika --- početni i krajnji~\eqref{d:81}. Time dobivamo konfiguraciju $(q_8,l+1,\t\$\bl^l\,v\t\$\bl\ldots)$, gdje je $l:=\dulj w+\dulj u+1$.
...
\label{d:82}\delta(q_8,\alpha):=(m_\alpha,\bl,-1)\text{, za sve }\alpha\in\Sigma_0\\
\label{d:83}\delta(m_\alpha,\bl):=(m_\alpha,\bl,-1)\text{, za sve }\alpha\in\Sigma_0\\
\label{d:84}\delta(m_\alpha,\t\$):=(q_8,\alpha,1)\text{, za sve } \alpha\in\Sigma_0\\
\label{d:85}\delta(m_\alpha,\alpha'):=(l_\alpha,\alpha',1)\text{, za sve }\alpha,\alpha'\in\Sigma_0\\
\label{d:86}\delta(l_\alpha,\bl):=(q_8,\alpha,1)\text{, za sve }\alpha\in\Sigma_0\\
...
\text{nakon treće faze}      & s_n & \dulj w+2 & \renewcommand{\arraystretch}{0.1}
    \t\$\bl^{\dulj w}\t\#\begin{array}{@{}c@{}}\text\textbullet\\?\end{array}^{\kr{v}}?^{z-\kr{v}}\bl\ldots\\
\text{nakon četvrte faze}    & q_5 & \dulj w+1 &
\renewcommand{\arraystretch}{0.1}
\t\$\bl^{\dulj w}\t\#\begin{array}{@{}c@{}}\text\textopenbullet\\?\end{array}^{z-\dulj v}v\t\$\bl\ldots\\
...
\renewcommand{\arraystretch}{0.1}
\T{\$\boo\boo\#}{p_0}{\boi}{\boi\boi\boi}\\
    \text{raspisano u~\eqref{eq:+a2}}&&{}\leadsto^*{}&
\renewcommand{\arraystretch}{0.1}
 \T{\$\boo\boo\#}{s_5}{\bio}{\bio\bio\bio\bio\bio\bio\bio\bio}\\
9=(121)_2=\kr{\t{aba}}&&{}\leadsto^*{}&
\renewcommand{\arraystretch}{0.1}
 \T{\$\boo\boo}{q_5}{\#}{\boo\boo\boo\boo\boo\boo aba}\\
\text{uokvirivanje izlaza}&&{}\leadsto^*{}&
\renewcommand{\arraystretch}{0.1}
\T{\$\boo\boo\boo\boo\boo\boo\boo\boo\boo}{q_8}{a}{ba\$}\\
\text{premještanje izlaza}&&{}\leadsto^*{}&
\renewcommand{\arraystretch}{0.1}
\T{aba\boo\boo\boo\boo\boo\boo\boo\boo\boo\boo}{q_8}{\$}{}\\
\text{izlaz}&&{}\leadsto^*{}&
\T{aba}{q_z}{\bl}{}
\end{align}
