...
\label{eq:defdigit}
    \f{digit}(n,i,b)&:=\f{mod}'\bigl(\f{prefix}(n,i+1,b),b\bigr)
\end{align}
...
Postoji parcijalno rekurzivna funkcija $\f{stop}^1$ takva da za svaku riječ $w\in\Sigma_0^*$ vrijede sljedeće dvije tvrdnje:
\begin{enumerate}
    \item\label{it:stop} Ako $\mathcal T_0$-izračunavanje s $w$ stane nakon $n_0$ koraka, tada je $\f{stop}(\kr w)=n_0$.
    \item\label{it:nstop} Ako $\mathcal T_0$-izračunavanje s $w$ ne stane, tada $\kr w\not\in\dom{\f{stop}}$.
\end{enumerate}
\end{lema}
\begin{proof}
...
Za tvrdnju~\ref{it:stop}, prema lemi~\ref{lm:StateHeadTapeprn} vrijedi $\f{State}(\kr w,n_0)=\N Q_0(q_z)=1$, ali isto tako za sve $n<n_0$ vrijedi $\f{State}(\kr w,n)\not=1$, jer stanje još nije završno, a $\N Q_0$ je injekcija. Dakle $n_0$ je jednak $\mu n\bigl(\f{State}(\kr w,n)=1\bigr)$, što je jednako $\f{stop}(\kr w)$ prema~\eqref{eq:stopdef}.
...
Fragment Turingovog stroja koji mu odgovara (treći fragment) je prikazan dijagramom
\begin{equation}
...
\end{lema}
\begin{proof}
Prvi korak se sastoji u brisanju svih ostataka izračunavanja, i separatora, tako da na traci ostanu jedino izlazna riječ $v$ i dva graničnika --- početni i krajnji~\eqref{d:81}. Time dobivamo konfiguraciju $(q_8,l+1,\t\$\bl^l\,v\t\$\bl\ldots)$, gdje je $l:=\dulj w+\dulj u+1$.
...
\renewcommand{\arraystretch}{0.1}
\T{\$\bl\bl\#}{p_0}{\boi}{\boi\boi\boi}\\
    2\cdot4+1=9\text{; raspisano u~\eqref{eq:+a2}}&&{}\leadsto^*{}&
\renewcommand{\arraystretch}{0.1}
 \T{\$\bl\bl\#}{s_5}{\bio}{\bio\bio\bio\bio\bio\bio\bio\bio}\\
