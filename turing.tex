...
\noindent\begin{gather}
\label{d:21}\delta(q_0,\alpha_t):=(q_1,\alpha_{t-1},1)\text{, za sve }t\in[2\dd b']\\
\label{d:22}\delta(q_1,\alpha):=(q_1,\alpha,1)\text{, za sve }\alpha\in\Sigma_0\mathbin{\dot\cup}\{\t\#,r_1\}\\
\label{d:23}\delta(q_1,\bl):=(q_0,r_1,-1)\\
\label{d:24}\delta(q_0,\gamma):=(q_0,\gamma,-1)\text{, za }\gamma\in\{\t\#,r_1\}\\
    \label{d:25}\delta(q_0,\alpha_1):=(q_0,\alpha_{b'},-1)\\
\label{d:26}\delta(q_0,\gamma):=(q_2,\gamma,1)\text{, za }\gamma\in\{\t\$,\bl\}\\
\label{d:27}\delta(q_2,\alpha_{b'}):=(q_1,\bl,1)\\
\label{d:28}\delta(q_2,\t\#):=(p_0,\t\#,1)
\end{gather}
...
\begin{korolar}[{name=[treći fragment transpiliranog stroja]}]
...
koja se prirodno nastavlja na onu započetu u~\eqref{eq:+afaze} (kroz lemu~\ref{lm:pi>si} za $i=0$).
    ...
    Treći fragment Turingovog stroja koji odgovara $P_{\t a}$ je prikazan dijagramom
\begin{equation}
...
\label{d:72}\delta(s_n,\bl):=(q_3,\t\$,-1)\\
\label{d:73}\delta_{(0)}(q_3,\t\textopenbullet):=(q_3,\t\textopenbullet,-1)\\
\label{d:74}\delta_{(0)}(q_3,\t\textbullet):=(q_4,\t\textopenbullet,1)\\
\label{d:75}\delta(q_4,\alpha):=(q_4,\alpha,1)\text{, za sve }\alpha\in\Sigma_0\dcup B^m\\
\label{d:76}\delta(q_4,\t\$):=(q_5,\t\$,-1)\\
\label{d:77}\delta(q_5,\alpha_{b'}):=(q_5,\alpha_1,-1)\\
\label{d:78}\delta(q_5,\alpha_t):=(q_3,\alpha_{t+1},-1)\text{, za sve }t\in[1\dd b'\rangle\\
\label{d:79}\delta(q_5,\gamma):=(q_3,\alpha_1,-1)\text{, za sve }\gamma\in B^m\\
\label{d:70}\delta(q_3,\alpha):=(q_3,\alpha,-1)\text{, za sve }\alpha\in\Sigma_0
\end{gather}
...
\noindent\begin{gather}
    \SwapAboveDisplaySkip
\label{d:81}\delta(q_3,\t\#):=\delta(q_6,\gamma):=(q_6,\bl,1)\text{, za sve }\gamma\in B^m\\
\label{d:83}\delta(q_6,\alpha):=\delta(m_\alpha,\bl):=(m_\alpha,\bl,-1)\text{, za sve }\alpha\in\Sigma_0\\
    \label{d:85}\delta(m_\alpha,\beta):=(l_\alpha,\beta,1)\text{, za sve }\alpha\in\Sigma_0\dcup\{\bl\},\beta\in\Sigma_0\\
\label{d:86}\delta(m_\alpha,\t\$):=\delta(l_\alpha,\bl):=(q_6,\alpha,1)\text{, za sve }\alpha\in\Sigma_0\\
\label{d:87}\delta(q_6,\t\$):=\delta(m_{\bl},\bl):=(m_{\bl},\bl,-1)\\
%\label{d:88}\delta(m_{\bl},\alpha):=(l_{\bl},\alpha,1)\text{, za sve }\alpha\in\Sigma_0\\
\label{d:89}\delta(m_{\bl},\t\$):=(l_{\bl},\bl,-1)
\end{gather}
...
\end{align}
i funkcijom prijelaza $\delta$ zadanom jednakostima \eqref{d:11}--\eqref{d:12}, \eqref{d:21}--\eqref{d:28}, \eqref{d:31}--\eqref{d:61}, \eqref{d:71}--\eqref{d:70} i \eqref{d:81}--\eqref{d:89}, tako da se svi parovi $(q,\alpha)\in(Q\setminus\{l_{\bl}\})\times\Gamma$ nenavedeni u tim jednakostima preslikavaju u $(q_x,\alpha,-1)$.
