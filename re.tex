...
Na prvi pogled, treba nam izračunljiva bijekcija između $\N^2$ i $\N$, s izračunljivim inverzima (tzv.\ \emph{pairing function}), ali pokazat će se da je injekcija dovoljna. Drugim riječima, samo nam treba neko kodiranje $\N^2$. Napravili smo dva: $\f{Code}^2$ i $\f{bin}^2$. Zaista je svejedno koje od njih (ili neko treće) koristimo, pa možemo to uobličiti kao sučelje (\emph{interface}) za neki apstraktni tip podataka (\texttt{std::pair<unsigned,unsigned>}).
...
Mogli bismo postaviti pitanje: možemo li naći relaciju takvu da \emph{niti} ona niti njen komplement nisu rekurzivno prebrojive? Pokušajte dokazati da relacija $B^2$, zadana s $B(x,y):\Longleftrightarrow DR(x)\leftrightarrow DR(y)$, ima to svojstvo.

\section{Veza sa slikama: teorem enumeracije}
...
Neka je $R$ jednomjesna brojevna relacija (drugim riječima, $R\subseteq\N$). Tada su sljedeće tvrdnje ekvivalentne:
\begin{enumerate}
    \item[\t{(1)}] $R$ je rekurzivno prebrojiva;
...
\begin{napomena}
Enumeracija $\f G$ koju smo konstruirali je dvomjesna: ako baš želimo \emph{niz}, nije problem definirati ga kontrakcijom: $\f F(n):=\f G\bigl(\f{fst}(n),\f{snd}(n)\bigr)$.
\end{napomena}
