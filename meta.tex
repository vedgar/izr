...
\begin{itemize}
    ...
    \item Bazu primitivne rekurzije~\eqref{eq:defprb} možemo zapisati kao $spec(0,G\pr H)=G$.
    \item Prema~\eqref{eq:resultdef}, $\f{result}=spec(0,\f{ex})$.
...
\begin{definicija}[{name=[dijagonala parcijalno rekurzivne funkcije]}]
	...
	\emph{Dijagonala} funkcije $\f H$ je $k$-mjesna funkcija $d\f H:=spec(h,\f H)$,\\ gdje je $h$ jedan (fiksni) indeks funkcije $\f H$.
\end{definicija}

Kako funkcija može imati više indeksa, može imati i više dijagonala --- odnosno formalno, dijagonala bi se trebala definirati za \emph{indeks}, ne za funkciju. No nama će prvenstveno biti bitna prateća funkcija $\f D=\N d$, koja je ionako definirana na indeksima.
