...
Prethodnik mjesnosti funkcije $\f S$ govori koliko njenih zadnjih argumenata želimo fiksirati (i moramo navesti njihove vrijednosti u pozivu prije njenog indeksa $e$), a supskript govori koliko još argumenata naša funkcija $\kf e$ prima (odnosno koliko će ih "preživjeti" specijalizaciju). Recimo, u slučaju funkcije mjesnosti $7$ i indeksa $e_1$, kojoj želimo fiksirati zadnja tri argumenta redom na brojeve $2$, $8$ i $5$, imali bismo
$
...
\end{equation*}
broj koji jest oveći (ima $7\,138\,041$ znamenku), ali ipak sasvim točno određen. Sad samo treba napisati $\f{plus}_k(e,f):=\f{compose}_k(e,f,a)$, odnosno $\f{plus}_k^2=spec(a,\f{compose}_k^3)$ je primitivno rekurzivna.
\end{proof}
...
\begin{definicija}[{name=[dijagonala parcijalno rekurzivne funkcije]}]
    Neka je $k\in\N_+$, te $\f H^{k+1}$ parcijalno rekurzivna funkcija. \emph{Dijagonala} funkcije $\f H$ je $k$-mjesna funkcija $d\f H:=spec(h,H)$, gdje je $h$ jedan (fiksni) indeks funkcije $\f H$.
\end{definicija}
...
\begin{propozicija}[{name=[primitivna rekurzivnost specijalizacija Ackermannove funkcije]}]\label{pp:Anprn}
...
Ako pak $e\not\in S$, tada je $F(e)=s\in S$, a zbog simetričnosti relacije $\approx_k$ imamo i $F(e)\approx_k e$. No to bi značilo da $S$ nije $k$-invarijantan, jer smo našli $s\in S$ takav da je $s\approx_k e$, ali $e$ nije u $S$.
...
\begin{primjer}[{name=[neodlučivost $k$-mjesnog brojevnog problema zaustavljanja]}]
    Za svaki $k\in\N_+$, skup $Halt_k$ nije rekurzivan. Naime, ako definiramo $T(e):\Longleftrightarrow Halt_k(\vec 0,e)$, očito je $T=\knk{\{\f f\in\mathcal Comp_k\mid\vec 0\in\dom{\f f}\}}$ $k$-invarijantan po lemi~\ref{lm:kinv=sind}, te vrijedi $0\not\in T$ i $1\in T$. Dakle $T$ nije rekurzivan, pa zbog $Halt_k\preceq T$ niti $Halt_k$ nije rekurzivan.
\end{primjer}
