...
\begin{itemize}
    \item Funkcija $\f{Lh3}$ iz leme~\ref{lm:Lh3} dobivena je parametrizacijom iz funkcije $\f{Lh}$, fiksirajući njen zadnji (drugi) argument na $3$. Skraćeno kažemo da je $\f{Lh3}$ parametrizacija od $\f{Lh}$ brojem $3$, i pišemo $\f{Lh3}=param(3,\f{Lh})$.
    \item Binarna operacija $\conc{13}$ je parametrizacija funkcije $\f{Concat}$ (lema~\ref{lm:Concatprn}) brojem $13$.
    \item Općenito u primitivnoj rekurziji~\eqref{eq:defprb}, baza je zapravo parametrizacija definirane funkcije nulom: $G=param(0,F\pr G)$.
...
Ipak, ta tvrdnja nije glavna tema ove točke. Konkretizirajući izračunljivost kroz postojanje indeksa, ona samo kaže da ako $\f f$ ima indeks, tada i $\f g$ ima indeks --- ne kaže, bar ne direktno, kako \emph{izračunati} indeks od $\f g$ pomoću indeksa od $\f f$. Primijetimo samo da nije veliki problem izračunati (neke) indekse od $\f I_i^k$ za $i\in[1..k]$, i od $\f C_y^k$ --- te kad bismo imali $\f{compose}$ kao izračunljivu funkciju na indeksima, mogli bismo kao specijalni slučaj dobiti i indeks kompozicije~\eqref{eq:paramcompose}. Ipak, lakše je prvo napraviti parametrizaciju u obliku izračunljive funkcije $\f S$ (dolazi od \emph{substitute} ili \emph{specialize}, ovisno o tome pitate li logičare ili programere), a onda $\f{compose}$ pomoću te funkcije.

Na neki način, $\f S=\N param$, gdje funkcije kodiramo njihovim indeksima na način koji je detaljno objašnjen pri kraju točke o indeksima~\eqref{eq:parcspecind}. Ukratko, $\f S$ prima $y$ i \emph{bilo koji} indeks za $\f f$, te vraća \emph{neki} indeks za $param(y,\f f)$.

Bilo bi super kad bismo mogli dobiti $\f S$ kao jednu funkciju, neovisnu o mjesnosti $k$ funkcije $\f f$ --- no lako se vidi da to, bar ovako kako smo zamislili indekse, nije moguće. Ta funkcija bi trebala primati $y$ i kod RAM-programa $e$, i vraćati kod novog RAM-programa $e'$ takvog da bude $\kf e(\vec x,y)\simeq\kf{e'}(\vec x)$ za sve $\vec x\in\N^k$. No to znači da \emph{mjesto} na kojem će pisati ulazni podatak $y$ u tom RAM-programu (registar $\reg{k+1}$) ovisi o $k$, a $k$ ne možemo zaključiti iz samog RAM-programa odnosno njegovog koda $e$.

Zapravo, mogli bismo dokazati i "uniformnu" verziju, kad bismo fiksirali argumente s početka a ne s kraja. Tada bi $y$ uvijek išao u $\reg1$, a sve ostale registre (koji se stvarno koriste u programu, što možemo zaključiti iz $e$) bismo pomakli za jedno mjesto udesno. To se može izvesti (i može poslužiti kao osnova za malo ozbiljniji studentski rad), ali je puno kompliciranije, a nama će ova verzija biti dovoljna.
...
Koristeći komponentne funkcije $\f{dest}$ i $\f{regn}$, te tipove instrukcija (leme~\ref{lm:regndestprn} i~\ref{lm:InsTYPEprn}), lako je $\f{Shift}$ definirati grananjem:
\begin{equation}
...
\begin{primjer}
Drugi subskript funkcije $\f S$ govori koliko zadnjih argumenata želimo parametrizirati (i moramo navesti njihove vrijednosti prije indeksa $e$), a prvi subskript govori koliko još argumenata naša funkcija $\kf e$ prima (odnosno koliko će ih "preživjeti" parametrizaciju). Recimo, u slučaju funkcije mjesnosti $7$ i indeksa $e_1$, kojoj želimo parametrizirati zadnja tri argumenta redom na brojeve $2$, $8$ i $5$, imali bismo
$
...
\subsection{Komponiranje izračunljivih funkcija je primitivno rekurzivno}

Sada možemo i vidjeti da je $\N compose$, bar za fiksne mjesnosti $k$ i $l$, primitivno rekurzivna. Tehnika koju ćemo pritom koristiti može se općenito primijeniti na razne funkcije definirane na funkcijama, koje su kodirane preko njihovih indeksa.

Primijetimo da ovdje \emph{ne} govorimo o primitivnoj rekurzivnosti kompozicije (rezultata komponiranja), niti o primitivnoj rekurzivnosti funkcija koje ulaze u kompoziciju. Radi ilustracije uzmimo $k=l=1$: tada nas ne zanima jesu li $\f G$ i $\f H$ (pa time i $\f F:=\f G\circ\f H$) primitivno rekurzivne. Sve što nas zanima je da su \emph{parcijalno} rekurzivne, odnosno da imaju indekse (redom ih označimo s $g$, $h$ i $f$). Govorimo o primitivnoj rekurzivnosti funkcije $\f{compose}_{11}$ koja uzima $g$ i $h$, te vraća $f$ --- pogledajte tamo negdje oko~\eqref{eq:parcspecind} ako ste zaboravili na što se tu misli.

Ugrubo, \emph{kompajlirati} kompoziciju dvije funkcije koje već imamo kompajlirane, je daleko regularnije nego \emph{izvršiti} (evaluirati) tu kompoziciju. Konkretno, izvršavanje ne mora stati, ako te funkcije nisu totalne. Sâmo kompajliranje kompozicije, s druge strane, mora stati --- i to u unaprijed specificiranom broju koraka.
...
\end{equation*}
broj koji jest malo poveći (ima $7\,138\,041$ znamenku), ali je ipak sasvim precizno određen. Sada samo treba napisati $\f{plus}(e,f):=\f{compose}_{k2}(e,f,a)$, odnosno $\f{plus}=param(a,\f{compose}_{k2})$ je primitivno rekurzivna.
\end{proof}
...
\begin{equation}
    ...
    \end{cases}\text,
\end{equation}
i sad bismo mogli upotrijebiti teorem~\ref{tm:gprek}, da nije jednog malog problema: jednadžba~\eqref{eq:defprk} ima traženu funkciju i s lijeve i s desne strane. Dakle, ovaj $f$ koji se nalazi u grani "inače", to je upravo onaj $primRecurse_k(g,h)$ kojeg želimo odrediti. Dakle, nemamo ga još, pa ga ne možemo koristiti u definiciji od $F_k$.

Ili možemo? Što bi se dogodilo da $f$ jednostavno proglasimo još jednim argumentom funkcije $F_k$? Imali bismo
\begin{equation}
    F_k(\vec x,y,g,h,f):\simeq\begin{cases}
        \f{comp}_k(\vec x,g),&y=0\\
        \f{comp}_{k+2}\bigl(\vec x,\f{pd}(y),\f{comp}_{k+1}\bigl(\vec x,\f{pd}(y),f\bigr),h\bigr),&\text{inače}
    \end{cases}\text,
\end{equation}
i po teoremu~\ref{tm:gprek} to je parcijalno rekurzivna funkcija, te ima indeks~\ldots\ i tako dalje. No sad bismo iz $F_k^{k+4}$ trebali parametrizirati $f$, a to ne znamo kako učiniti jer još uvijek ne znamo koju vrijednost staviti za $f$.
...
Naravno, netrivijalan je samo u formalnom smislu: intuitivno, lako je objasniti kako izvršavati opću rekurzivnu funkciju (za zadanu funkciju $\f G$), i taj neformalni algoritam po Church--\!Turingovoj tezi računa parcijalno rekurzivnu funkciju. Ipak, to intuitivno objašnjenje može lako navesti na krivi trag (sjetite se funkcije $h_1$), a i tvrdnje o jedinstvenosti i rekurzivnosti lakše je dokazivati (najčešće matematičkom indukcijom) jednom kad imamo indeks, nego kad imamo neformalni postupak. Zato je dobro izbjeći Church--\!Turingovu tezu, i doista konstruirati indeks $e$ s traženim svojstvom.

\subsection{Dijagonalna funkcija}

Da bismo to učinili, zapitajmo se koja svojstva mora imati funkcija $\f F=\kf e^k$. Na neki način, ona mora "znati svoj indeks", odnosno pozvana s argumentima $\vec x$, mora se ponašati kao funkcija $\f G$ pozvana s $\vec x$ i još indeksom $e$.
...
Zapravo, C je ovdje izuzetno zanimljiv, jer na mnogim kompajlerima, ako ignorirate \emph{warning}, zapravo ćete dobiti ispravno rješenje (pokušajte na svom omiljenom kompajleru). Kako je to moguće?! Zar je C kompajler uspio dokazati teorem rekurzije prije nas?
...
Možemo li to iskoristiti da bismo eksplicitno napisali taj poziv u kodu, i izbjegli \emph{warning}? Skoro, ali ipak ne. Naravno, napisati poziv \t{f(n-1,G)} je beskorisno jer bismo opet imali rekurzivni poziv, ali umjesto toga možemo napisati "dijagonalni" poziv \t{f(n-1,f)} koji (bar sintaksno) nije rekurzivan. Razlog zašto ne možemo izbjeći \emph{warning} leži u tome da ne možemo nikako precizirati tip drugog argumenta. Htjeli bismo reći nešto poput
\begin{equation}
    \t{typedef unsigned funkcija(unsigned n, funkcija f);}
\end{equation}
--- ali standard jezika C ne dozvoljava takve rekurzivne definicije tipova. "Rješenje" je poslati \emph{void pointer}, pa ga ručno \emph{castati} gdje treba, ali onda je jednostavnije prebaciti se na dinamički tipiziran jezik. U Pythonu, stvari rade točno kako smo zamislili:
\begin{verbatim}
            >>> def H(n, f):
...
Naravno, možemo i samo dijagonaliziranje (koje koristimo dvaput --- jednom u definiciji, i jednom u pozivu, funkcije \t H) izdvojiti u zasebnu funkciju.
\begin{verbatim}
            >>> def D(H):
            ...
            5040
\end{verbatim}
Dobra strana toga je da više ne moramo ručno proizvoditi funkciju \t H iz funkcije \t G. Pretodno smo to napravili tako što smo, tamo gdje je \t G pozivala \t f, u funkciji \t H pozvali $d\t f$. Sad to možemo formalizirati, tako da definicija od \t H pozove \t G s $d\t f$ automatski.
\begin{verbatim}
            >>> def G(n, f):
            ...     if n: return n * f(n-1)
            ...     else: return 1
...
I to je dokaz teorema rekurzije (za $k=1$)! Ako pažljivo promotrimo, vidjet ćemo da nigdje nismo koristili rekurzivne pozive. Samo sad sve to treba napisati matematički.
...
Recimo, ako se totalnost od $\f F_0^1$ može dokazati običnom matematičkom indukcijom, to znači da imamo neku ogradu za broj koraka računanja $\f F_0(0)$, te neki izračunljiv način da iz ograde za broj koraka u računanju $\f F_0(n)$ dobijemo takvu ogradu za $\f F_0(n+1)$ --- a to zapravo znači da $F_0$ možemo dobiti (degeneriranom) primitivnom rekurzijom. Ako umjesto obične indukcije moramo upotrijebiti jaku indukciju, to na isti način znači da se $F_0$ može dobiti rekurzijom s poviješću. U slučaju simultane indukcije, imamo simultannu rekurziju, i tako dalje.

Vjerojatno najjednostavniji obrazac indukcije koji je nemoguće uloviti u teoriji primitivno rekurzivnih funkcija je \emph{ugniježđena} indukcija po dvije varijable, gdje iz pretpostavke da tvrdnja vrijedi za neki $m$ i za \emph{sve} $n$, slijedi da vrijedi za $m+1$ i za sve $n$. Jedan od prvih primjera takve funkcije (čija totalnost zahtijeva takav indukcijski obrazac koji nije formalizabilan primitivnom rekurzijom) dao je Ackermann 1928.
...
\begin{napomena}
Često se u literaturi navodi pojednostavljena, dvomjensna funkcija $\f A^2$, zadana s
\begin{align}
\SwapAboveDisplaySkip
    \f A(0,n)&:=n+1\text,\\
...
\end{equation}
iz čega slijedi da kad bi $\f A^3$ bila primitivno rekurzivna, bila bi takva i $\f A^2$. No $\f A^2$ nije primitivno rekurzivna (dokaz možete vidjeti u~\cite{skr:Vuk}) pa kontrapozicijom slijedi da ni $\f A^3$ nije primitivno rekurzivna.
\end{napomena}
...
\begin{equation}
    ...
    \end{cases}\text,
\end{equation}
a zatim to napišimo u obliku $\f{comp}_k(x,y,z,e)=\f G(x,y,z,e)$, gdje je očito $k=3$, $e$ je traženi indeks, a funkcija $\f G^k$ je zadana s
\begin{equation}
...
\end{equation}
što je u slučaju $t=0$ jednako $\f{Astart}(x,1)=x=x+0=\f A_1(x,0)=\f A(x,0,1)$, u slučaju $t=1$ je jednako $\f{Astar}(x,2)=0=x\cdot0=\f A_2(x,0)=\f A(x,0,2)$, a u svim ostalim slučajevima ($t\ge2$) je jednako $\f{Astar}(x,t+1)=1=\f A_{t+1}(x,0)=\f A(x,0,t+1)$. Dakle, uvijek je $\kf a(x,0,t+1)\simeq\f A(x,0,t+1)$, pa je unutarnja baza dokazana.
...
\section{Teorem o fiksnoj točki}
