...
\begin{itemize}
    \item Funkcija $\f{blh}$ iz leme~\ref{lm:blh} dobivena je specijalizacijom funkcije $\f{lh}$, fiksirajući njen zadnji (drugi) argument na $2$. Skraćeno kažemo da je $\f{blh}$ $2$-specijalizacija $\f{lh}$, i pišemo $\f{blh}=spec(2,\f{lh})$.
...
\begin{korolar}\label{kor:paramprn}
    Ako je $k\in\N_+$, $\f f^{k+1}$ (primitivno) rekurzivna, te $y\in\N$, tada je i $\f g^k=spec(y,\f f)$ (primitivno) rekurzivna.
\end{korolar}
\begin{proof}
