...
Injektivnost je lako dokazati: neka su $\varphi_1,\varphi_2\in\TComp$ različite. Ako imaju različitu domenu, bez smanjenja općenitosti možemo pretpostaviti da postoji $w\in\dom{\varphi_1}\!\setminus\dom{\varphi_2}$. Tada je $\kr w\in\dom{\N\varphi_1}$, ali isto tako $\kr w\not\in\dom{\N\varphi_2}$ jer je kodiranje riječi injekcija, pa $\kr w$ ne može biti jednak nijednom $\kr{w'}$ za $w'\in\dom{\varphi_2}$. To znači da prateće funkcije $\N\varphi_1$ i $\N\varphi_2$ imaju različite domene, pa su različite.
...
Vjerojatno ste nekad napisali Turingov stroj za $\beta\f{add}^2$, koristeći $\beta\f{add}^2(u\t/v)=uv$, ako su $u,v\in\t\textbullet^*$. Ipak, već tada ste zasigurno vidjeli koliko je teško napisati $\beta\f{mul}^2$, a pogotovo $\beta\f{pow}$ --- ostale divote iz poglavlja~\ref{ch:univ} ($\beta\f{plog}$?!) da i ne spominjemo.
...
\begin{definicija}\label{def:bink}
...
\begin{lema}\label{lm:bitcount}
Definiramo funkciju $\f{bitcount}$ s $\f{bitcount}(n):=\f{lh}(n,2)$. 

Funkcija $\f{bitcount}$ je primitivno rekurzivna, i vrijedi $\f{bitcount}\circ\f{bin}^1\!=\f I_1^1$.
\end{lema}
\begin{proof}
Primitivna rekurzivnost slijedi iz simboličke definicije $\f{bitcount}=\f{lh}\circ(\f I_1^1,C_2^1)$, leme~\ref{lm:ldcpprn} i propozicije~\ref{prop:konst}.
Za kompoziciju, uzmimo proizvoljni $n\in\N$, i računamo $\f{bitcount}\bigl(\f{bin}(n)\bigr)=\f{bitcount}(2^n-1)$. Relacija koju minimiziramo (po $t\le 2^n-1$) je
\begin{equation}
...
Na komutirajućem dijagramu koji cijelo vrijeme promatramo, $\f{bitcount}$ bismo mogli prikazati kao strelicu prema gore, s desne strane pravokutnika, ispupčenu još više nego strelica $\f{bin}^1$. Upravo dokazana lema pokazuje da tako dobiveni "polumjesec" također komutira, pa i "proširena elipsa" također komutira --- čime smo napokon prikazali $f$ pomoću $\N\beta f$.

\begin{teorem}\label{tm:btip}
Neka je $k\in\N_+$, i\, $\f f^k$\! funkcija takva da je $\beta\f f$ Turing-izračunljiva. Tada je $\f f$ parcijalno rekurzivna.
\end{teorem}
\begin{proof}
Prema teoremu~\ref{tm:tikp}, primijenjenom na abecedu $\Sigma_\beta$, kodiranje $\t\textbullet\mapsto1,\t/\mapsto2$ i funkciju $\beta\f f$, funkcija $\N\beta\f f$ je parcijalno rekurzivna. Iz leme~\ref{lm:bitcount} i korolara~\ref{kor:binf=Nbetafbin} sada imamo
\begin{equation}
    \f f=\f I_1^1\circ\f f=\f{bitcount}\circ\f{bin}^1\!\circ\f f=\f{bitcount}\circ\N\beta\f f\circ\f{bin}^k\text,
\end{equation}
odnosno $\f f$ je dobivena kompozicijom iz tri funkcije, od kojih je srednja parcijalno rekurzivna, a prva i zadnja su primitivno rekurzivne (lema~\ref{lm:bitcount} i propozicija~\ref{pp:binkprn}), pa su rekurzivne (korolar~\ref{kor:prnrek}) a time i parcijalno rekurzivne. Kako je skup parcijalno rekurzivnih funkcija zatvoren na kompoziciju, $\f f$ je parcijalno rekurzivna.
\end{proof}
...
\end{propozicija}
\begin{proof}
Za refleksivnost, dovoljno je staviti $u:=\varepsilon$. Za tranzitivnost, samo treba iskoristiti asocijativnost konkatenacije: ako je $w=vu$ i $v=v'u'$, tada je $w=(v'u')u=v'(u'u)$, pa je $v'$ prefiks od $w$.
...
primitivna rekurzivnost iz leme~\ref{lm:ldcpprn}: $x\sqsubseteq_\beta y\Longleftrightarrow x=\f{prefix}\bigl(y,\f{bitcount}(x),2\bigr)$.
\end{proof}
...
\begin{labeling}{$\f{streak1}(x,i)$}
    \item[$\f{pos2}(x,i)$] je pozicija $i$-te dvojke (ili\, $\f{bitcount}(x)$ ako takva ne postoji), te
...
\begin{equation}
    \f{count2}(x):=\bigl(\#i<\f{bitcount}(x)\bigr)\bigl(\f{digit}(x,i,2)=2\bigr)
\end{equation}

Tvrdimo da funkcije zadane točkovno s
\begin{align}
    \f{pos2}(x,i)&:=\f{bitcount}\bigl((\mu z<x)(z\conc22\,\sqsubseteq_\beta x\,\land\,\f{count2}(z)=i)\bigr),\\
...
Za $i\ge\f{count2}(x)$, takav $z$ ne postoji (jer $z\conc22$ ima više dvojki nego $x$, pa ne može biti $z\conc22\sqsubseteq_\beta x$), te minimizacija $(\mu z<x)$ dade $x$, odnosno $\f{pos2}(x,i)=\f{bitcount}(x)$, opet, kao što i treba biti po specifikaciji.
...
Za $i=\f{count2}(x)-1$, $i$-ta dvojka je upravo zadnja, pa je $\f{pos2}(x,i)=\f{bitcount}(x)-1$ (možemo pisati obične minuse jer uokvireni brojevi imaju bar dvije dvojke --- na početku i na kraju). Također po specifikaciji $\f{pos2}$ vrijedi $\f{pos2}(x,i+1)=\f{bitcount}(x)$, pa je $\f{streak1}(x,i)=\f{bitcount}(x)\ominus\f{Sc}\bigl(\f{bitcount}(x)-1\bigr)=0$.

Za $i\ge\f{count2}(x)$ imamo $\f{pos2}(x,i+1)=\f{pos2}(x,i)=\f{bitcount}(x)$, pa je opet $\f{streak1}(x,i)=\f{bitcount}(x)\ominus\f{Sc}\bigl(\f{bitcount}(x)\bigr)=0$.
\end{proof}
...
Kao što smo već rekli, dinamizacijom funkcija $\f{arg}_i$ možemo simulirati i funkcije koje primaju proizvoljan broj parametara. Definiramo pomoćne (primitivno rekurzivne) funkcije
\begin{align}
\SwapAboveDisplaySkip
    \f{argc}(a)&:=\f{Sc}\bigl(\f{count2}(a)\bigr)\text,\\
...
\end{equation}
primitivno rekurzivna po lemi~\ref{lm:sumrek} i prethodnim rezultatima.
\end{primjer}
